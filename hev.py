...
def hev(inp,par):
    ...
    Tg  = (inp.W[3]>0)  * (Tv>0)   * Tv / r_gear(inp.W[3] + (inp.W[3]==0)) / 0.95+ (inp.W[3]>0)  * (Tv<=0)  * Tv / r_gear(inp.W[3] + (inp.W[3]==0))  * 0.95
    ...
    inm = (isnan(e)) + (Tm<0)   * (Tm < interp1(wm_list,Tmmin,wg,'linear*','extrap')) +(Tm>=0)  * (Tm > interp1(wm_list,Tmmax,wg,'linear*','extrap'))
    e[isnan(e)] = 1
    ...
    r = (Pm>0)   * interp1(soc_list, R_dis, inp.X[1],'linear*','extrap')+ (Pm<=0)  * interp1(soc_list, R_chg, inp.X[1],'linear*','extrap')

    im = (Pm>0)  * 100 + (Pm<=0)  * 125
    v = interp1(soc_list, V_oc, inp.X[1],'linear*','extrap')

    Ib  =   e  * (v-math.sqrt(v**2 - 4 *r *Pm))/(2 *r)
    X[1]  = - Ib / (6 * 3600) + inp.X[1]

    Pb =   Ib  * v

    inb = (v**2 < 4 *r *Pm) + (math.fabs(Ib)>im)
